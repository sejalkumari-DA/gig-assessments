...
export const handleAudioAssessmentUpload = async (req: Request, res: Response) => {
  try {
...
Only return the JSON object.`;
    ...
    if (req.body.type === 'audio') {
        tableName = 'audio_assessments'; // Keeping consistent with the GET endpoint
    }
    ...
    if (insertError) {
      console.warn('Full audio assessment insert failed, attempting dummy db fallback:', insertError.message);
    }
...
export const generateAudioAssessmentQuestions = async (req: any, res: any) => {
  ...
  try {
    ...
    const jobDetails = req.query.jobDetails as string || '';
    
    let fetchedJD = '';
    if (jobId && jobId.length > 20) { // likely a UUID
      try {
        const env = req.headers['x-environment'] as string;
        const dbClient = getSupabaseClient(env);
    ...
    if (bedrockClient && jdText) {
      console.log('Generating audio questions for JD text/ID:', jdText.substring(0, 500) + '...');
      const prompt = `You are an expert technical recruiter and language detector. 
Analyze the following Job Description (JD) text:
"""
${jdText}
"""

First, detect the primary language of this JD. If it explicitly states a language (like Russian, Spanish, etc.), or is written in that language, use that language.

Then, generate EXACTLY 3 audio assessment questions. The content MUST be written ENTIRELY in the detected language of the JD. Do NOT write the content in English unless the JD is in English.

Format each question exactly like this:
1. "Cold-Reading Exercise: Please read the following script aloud.\\n\\n" followed by a short, professional paragraph (3-4 sentences) translated into the detected language.
2. "Cold-Reading Exercise: Please read the following script aloud.\\n\\n" followed by another short, professional paragraph translated into the detected language.
3. "Situational Question: Please answer the following question aloud.\\n\\n" followed by a relevant situational question translated into the detected language.

Return ONLY a valid JSON array of strings containing the 3 questions. 
CRITICAL: You MUST properly escape all newlines as \\n inside the JSON strings. Do NOT output raw unescaped line breaks inside the strings. No markdown formatting, no explanations, just the valid JSON array.`;

      const rawOutput = await callBedrock(prompt, true);
      console.log('Raw output from Bedrock for audio questions:', rawOutput);
      ...
      if (jsonMatch) {
        try {
          ...
          if (Array.isArray(questions) && questions.length === 3) {
            console.log('Successfully generated audio questions from AI');
            return res.json({ success: true, questions, source: 'ai' });
          } else {
            console.warn('AI generated JSON but length is not 3 or not an array:', questions);
          }
        } catch (parseErr) {
          console.error('Failed to parse JSON from Bedrock output:', parseErr);
        }
      } else {
        console.warn('No JSON array found in Bedrock output');
      }
    } else {
      console.log('Falling back to default questions. bedrockClient:', !!bedrockClient, 'jobId:', jobId);
    }

    return res.json({
      success: true,
      questions: defaultQuestions,
      source: 'hardcoded-audio'
    });
  } catch (err: any) {
    console.error('Failed to generate dynamic audio questions:', err);
    return res.json({
      success: true,
      questions: defaultQuestions,
